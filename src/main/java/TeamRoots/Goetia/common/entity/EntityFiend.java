    	...
    	if (getHealth() <= 0 && initHealth > 0){
    		if (source.getEntity() instanceof EntityPlayer){
    			if (((EntityPlayer)source.getEntity()).hasCapability(ImpurityProvider.impurityCapability, null)){
    				((EntityPlayer)source.getEntity()).getCapability(ImpurityProvider.impurityCapability, null).setImpurity((EntityPlayer)source.getEntity(), ((EntityPlayer)source.getEntity()).getCapability(ImpurityProvider.impurityCapability, null).getImpurity()+rand.nextInt(4)+5);
    			}
    		}
    	}
    	return true;
